...
type Counterpart = { id: string; name: string }
...
// 소상공인(buyer) 쪽 거래명세서 목록. RLS(deals_select_buyer_or_partner)가
// 이미 buyer 본인 거래만 select 가능하도록 막아주므로 이 화면은 그 위에서
// .eq('buyer_id', buyerProfile.id)로 조회만 하면 됨(파트너 쪽과 동일한
// 보호 - 새 정책 불필요, 20260907000000_quotes_deals_policies.sql 참고).
export default function MyPageStatementsPage() {
  ...
  const groups = useMemo(() => {
    const rows = deals
      ...
      .map((d) => ({
        ...
        counterpartId: d.partners!.id,
        counterpart: d.partners!,
      }))
  ...
  return (
    <div>
      ...
      <div style={styles.sectionSub}>거래처별로 발행된 거래명세서를 조회하고 인쇄할 수 있어요.</div>
...
function StatementRow({ group }: { group: StatementGroup<Counterpart> }) {
  ...
  return (
    <tr>
      ...
      <td style={styles.td}>{group.counterpart.name}</td>
