...
- **거래처 주문 마감시간 설정** (이번 작업, 아래 상세) — 공급업체가
  배송요일+주문마감시간을 묶은 "주문그룹"을 만들고 거래처를 배정하면,
  거래전표 등록/AI 빠른입력 저장 시점에 그 거래처가 배정된 그룹의
  마감시간을 기준으로 예상 배송일을 자동 계산해 화면에 안내하고
  `deal_line_items.expected_delivery_date`에 저장함. 마감 전/마감 후/
  시간외 허용/그룹 미지정 4가지 케이스 전부 실제 데이터로 검증함.
- **일자별 거래명세서 통합 처리** — "진행 중인
  거래" 목록에서 같은 확정일+같은 소상공인 거래를 한 행으로 병합(펼치면
  개별 거래 확인 가능), 명세서 인쇄도 그 그룹 안의 모든 거래 품목을 한
  장에 합쳐서 출력. `deals`/`deal_line_items` 테이블 구조는 그대로 두고
  화면 조회 단계에서만 그룹핑 — 실제로 같은 날짜 거래 3건을 만들어
  병합/펼침/통합 인쇄까지 브라우저에서 검증함.
- **거래전표 등록 그리드형 UI 재설계** — 마켓봄류
  유통관리 프로그램처럼 표 안에서 여러 품목을 한 번에 입력하는
  `LineItemGrid`(신규 공용 컴포넌트) 도입. Tab/Enter로 셀·행 이동,
  Insert 키로 행 추가, Ctrl+S/F8로 저장, 마지막 행에 입력하면 자동으로
  빈 행 추가. AI 빠른입력의 확인·수정 화면(Step 3)도 같은 컴포넌트를
  재사용 — 백엔드/데이터 구조는 전혀 안 건드림(순수 UI/UX 재설계).
- **"AI 거래전표 빠른입력" + "새 거래처로 시작하기"** (아래
...
지시 배경: 공급업체 사장님들이 익숙한 마켓봄류 유통관리 프로그램처럼
"표 안에서 여러 행을 한 번에" 입력하는 방식으로, 기존 "한 번에 한 품목씩
모달에 입력" 방식을 대체. **백엔드/데이터 구조는 전혀 안 건드림** —
`deal_line_items` 테이블도, insert 로직(같은 컬럼, 같은 방식)도, 트리거도
그대로. 순수 UI/UX 재설계.

### 라이브러리 없이 직접 구현

react-data-grid/ag-grid/handsontable 같은 그리드 라이브러리를 검토했지만
쓰지 않기로 함 — 컬럼이 5개(품목명/수량/단위/단가/외상)뿐인 단순한
표라 요구된 상호작용(Tab/Enter 이동, Insert로 행 추가, Ctrl+S/F8 저장)을
순수 React state + `<table>` + `ref` 기반 포커스 제어만으로 충분히 구현
가능했고, 이 프로젝트가 지금까지 어떤 UI 라이브러리도 안 쓰고 전부 직접
만들어온 관례와도 맞음(번들 크기·CSP 이슈도 없음).

### `components/LineItemGrid.tsx` (신규 공용 컴포넌트)

- **Tab/Shift+Tab**: 별도 구현 없음 — `<table>` 안에 `<input>`들이
  자연스러운 DOM 순서(행→열)로 배치돼 있어 브라우저 기본 동작만으로
  "다음 셀로 이동"이 이미 됨. 삭제 버튼만 `tabIndex={-1}`로 빼서 흐름을
  안 끊게 함.
- **Enter**: 같은 열의 다음 행으로 이동(지시 스펙 "Tab/Enter로 다음
  셀/다음 행"을 그대로 해석 - Tab=셀, Enter=행). 마지막 행에서 Enter를
  누르면 새 행을 만들고 그리로 이동.
- **Insert 키 / "+ 행 추가" 버튼**: 맨 끝에 빈 행 추가.
- **Ctrl+S 또는 F8**: `onSave` 콜백 호출(그리드 컨테이너에 이벤트 위임 -
  안 어떤 셀에 포커스가 있어도 동작).
- **마지막 행에 뭔가 입력되면 자동으로 빈 행 하나 더 추가**(엑셀처럼
  항상 입력 가능한 빈 줄이 마지막에 있도록) — Enter가 항상 "다음 행"을
  가질 수 있게 하는 전제이기도 함. 단, 이 자동 추가는 **그리드 내부
  편집(`updateRow`)을 거칠 때만** 발동함 - 부모가 `rows`를 통째로 설정하는
  최초 진입 시점(AI 파싱 결과를 그리드에 처음 채울 때 등)에는 안 걸려서,
  그런 경우 호출부가 직접 빈 행을 하나 더 넣어줘야 함(아래 AI 연동 참고).
- **저장은 그리드가 모름** — `onSave: () => void` 콜백만 있고, 실제
  `deal_line_items` insert(+ 그 이상의 로직)는 전부 호출부가 함. 그리드는
  순수 "행 상태 편집 + 키보드 내비게이션"만 담당 - 수동 입력과 AI
  빠른입력의 저장 동작이 서로 다르기 때문(아래 참고)에 의도적으로 분리.
- 제네릭(`<T extends GridRow>`)으로 만들어서, 기본 5개 필드 위에 호출부가
  자기만의 필드를 얹어 쓸 수 있음(AI 모달의 `raw_phrase`/
  `matched_existing_item`가 그 예 - 그리드 자신은 이 필드들의 존재를
  전혀 모름).

### 수동 입력 (`app/partner/dashboard/ledger-entry/page.tsx`)

기존 "품목명/수량/단위/단가/외상여부 각 1개 입력창 + 품목 추가 버튼"
(`addLineItem`)을 통째로 걷어내고 `<LineItemGrid>` 하나로 교체. 저장
시(`handleGridSave`) 품목명이 채워진 행만 걸러서(그리드가 항상 유지하는
마지막 빈 행은 자연히 제외됨) `deal_line_items.insert(filled.map(...))`
**배열 한 번**으로 일괄 등록 - 기존 트리거는 Postgres row-level 트리거라
배열 insert든 낱개 insert든 행마다 정확히 한 번씩 실행되는 것에 차이가
없음(건드릴 필요 자체가 없었음). 저장 성공 후 그리드는 빈 행 1개로
초기화, 아래 이력 표는 그대로 재조회.

### AI 빠른입력 연동 — 검토 결과: **같은 그리드를 그대로 재사용**

지시에서 "검토해서 알려달라"고 한 부분. 결론: 새 화면을 안 만들고
`AiQuickEntryModal`의 Step 3(확인·수정)이 수동 입력과 **완전히 같은
`LineItemGrid` 컴포넌트**를 씀 - AI가 만든 초안(`items.map(toEditableRow)`)
을 그리드의 `rows`에 그대로 채워 넣고, "확인 필요(신규 품목)" 노란 배경/
뱃지는 `rowStyle`/`renderRowBadge` prop으로 얹음(그리드 자신은
`matched_existing_item`이 뭔지 전혀 모름 - AI 모달 쪽에서만 해석).
장점: 편집 UX(Tab/Enter/Insert/자동 빈 행)가 수동 입력과 AI 초안 확인
화면에서 완전히 동일해져서, 사장님이 한 번 그리드 사용법을 익히면 두
화면 어디서나 그대로 통함. AI 모달의 기존 커스텀 표(별도 `updateRow`/
`removeRow`/`addRow` 로직 포함)는 전부 제거하고 그리드로 대체함 - 코드
중복도 없어짐.

**AI 쪽에서만 필요했던 보정 2가지** (실제 테스트로 발견):
1. AI 파싱 결과를 `setRows()`로 최초 설정할 때는 그리드의 "자동 빈 행
   추가"가 안 걸리므로(위 참고), `setRows([...items.map(toEditableRow),
   emptyReviewRow()])`처럼 빈 행을 직접 추가해서 넘겨야 함 - 안 하면
   AI 결과만 있고 빈 줄이 없어 이어서 수동으로 품목을 추가하려면 "+ 행
   추가" 버튼을 먼저 눌러야 하는 불편함이 있었음.
2. **실제로 겪은 버그**: `handleConfirm()`(저장 로직)이 그리드가 항상
   유지하는 마지막 빈 행까지 그대로 검증 대상에 포함시켜서, "품목명이
   비어있는 행이 있어요" 에러로 저장이 막히는 문제가 있었음(실제 브라우저
   테스트로 재현·발견). 수동 입력 쪽 `handleGridSave`가 이미 하던 것과
   동일하게 `rows.filter(r => r.item_name.trim())`로 빈 행을 걸러낸 뒤에만
   검증/저장하도록 고침. `wasEdited()`(AI 결과를 수정했는지 판단해
   `ai_parse_logs.was_edited`에 기록하는 함수)도 같은 이유로 원본
   `rows.length` 대신 필터링된 `filled.length`와 비교하도록 같이 고침(안
   그러면 그리드가 항상 붙여두는 빈 행 때문에 length가 항상 1 많아져서
   "수정 안 했는데도 항상 was_edited=true"로 잘못 기록될 뻔했음).

### 실제 검증 (puppeteer-core, 키보드 이벤트 시뮬레이션 관련 발견 포함)

test4@email.com 실제 계정, 실제 거래(all식당, 견적 기반)로 검증:
- 수동 그리드: 품목명 입력 → 자동 빈 행 추가(1→2→3행 확인) → Enter로
  같은 열 다음 행 이동 확인 → Insert 키로 행 추가 확인 → Ctrl+S로 저장 →
  이력 표에 정확히 반영 + 그리드가 빈 행 1개로 초기화되는 것까지 확인.
- AI 모달: 실제 카톡 문구("오이 8박스, 애호박 4개 2000원")를 실제
  Claude API로 분석 → 파싱 결과가 그리드에 채워지고 "확인 필요(신규
  품목)" 노란 배경/뱃지가 정확히 뜨는 것 확인 → 빈 단가 채운 뒤 그리드의
  Ctrl+S로 저장 → `deal_line_items`에 정확히 반영 + 모달이 닫히는 것까지
  확인.
- 콘솔 에러 0건. `tsc`/`lint` 새 에러 없음.

**⚠ 테스트 도구 관련 발견 (이 프로젝트를 다시 puppeteer로 테스트할 때
알아둘 것)**: 이번 환경에서 `page.keyboard.type()`/`page.keyboard.press()`
(Puppeteer의 CDP 기반 키보드 API)가 React의 합성 이벤트 시스템까지
안 이어지는 경우를 겪음(값 입력도, Enter/Insert/Ctrl+S 같은 특수키도
전혀 반응 없었음 - "0건 삭제, 에러 없음"과 비슷하게 조용히 실패해서
처음엔 컴포넌트 버그로 오해할 뻔함). `element.focus()` + 네이티브 setter
+ `dispatchEvent(new Event('input'))`(값 입력용, 이 세션에서 계속 써온
방식)와 `element.dispatchEvent(new KeyboardEvent('keydown', {key, ...,
bubbles:true}))`(특수키용, 이번에 새로 확립)로 **직접 이벤트를
dispatch**하면 정확히 동작함 - 실제 컴포넌트 로직은 처음부터 문제
없었고 순전히 이 환경의 Puppeteer 키보드 시뮬레이션 문제였음. 다음에
폼 입력이 아닌 키보드 단축키(Enter/Tab/Ctrl+조합 등)를 테스트할 때는
`page.keyboard.press()`를 먼저 믿지 말고, 이 방식으로 바로 넘어갈 것.

**실제 계정 오염 및 정리**: 위 검증 과정에서 test4의 **실제(견적 기반)
거래**(all식당, deal id `37dbc5d5-4dc2-4231-bd6b-a2b3a537f6e0`)에 테스트
품목 6건(E2E그리드품목A/B, 오이×2세트/애호박×2세트 - 그리드 빈 행 검증
버그 때문에 재시도가 필요해서 세트가 하나 더 생김)이 실제로 섞여
들어갔음. `deal_line_items`는 삭제 UI/정책이 없는 재무 기록이라
`supabase/migrations/20260917000000_cleanup_grid_test_line_items.sql`
(1회성, 20260916000000과 동일한 RLS 명시적 우회 패턴)로 정리 필요 -
**아직 사용자가 실행 전**이면 꼭 실행해주세요(정확한 대상 6개 행 id를
읽기 전용 조회로 먼저 확인한 뒤 작성함 - 진짜 품목인 냉동 흰살생선/
얼음/청경채 3건은 전혀 안 건드림).

## 일자별 거래명세서 통합 처리

지시 배경: "진행 중인 거래" 목록에서 같은 날짜에 같은 소상공인과 여러
번 거래하면(예: 2026.09.07에 핫한 핫도그와 3번 거래) 지금까지는 별도
행 3개 + 별도 "명세서 인쇄" 링크 3개로 나뉘어 보였음. 실제 유통 관행은
하루치 주문을 모아 다음날 한 번에 배송하고 정산도 월/보름 단위로 하므로,
같은 날짜+같은 거래처는 명세서도 한 장으로 합쳐서 뽑는 게 맞음.
**`deals`/`deal_line_items` 테이블 구조는 전혀 안 건드리고** 화면
조회(그룹핑) 단계에서만 처리 — 지시에서 요구한 그대로.

### 확인이 필요하다고 한 두 가지 — 조사 결과

1. **날짜 기준**: `deals`에는 `created_at`이 따로 없고 `confirmed_at`
   (timestamptz)만 있음 — 거래명세서 인쇄 페이지가 이미 이 값을 "거래일자"로
   써왔던 것과 동일하게, 그룹핑 키도 `confirmed_at`을 로컬 캘린더 날짜로
   변환한 값(`formatDate()` 결과, `YYYY.MM.DD`)을 씀.
2. **`/partner/ledger`의 "일자별 거래전표 내역"에 재사용할 그룹핑 로직이
   있는지**: **없었음.** 실제로 읽어보니 그 섹션은 `deal_line_items`를
   `created_at` 내림차순으로 그냥 나열만 할 뿐, 날짜+거래처로 묶어서
   합산하는 로직이 전혀 없음(테이블 헤더에 "일자"가 있어서 그룹핑된
   것처럼 보이지만 실제로는 행 단위 나열). 그래서 재사용하지 않고
   `app/partner/dashboard/deals/page.tsx`에 `groupDeals()`를 새로 작성함.

### 구현

**`app/partner/dashboard/deals/page.tsx`**
- `deals` select에 `buyer_id` 추가(그룹 키로 필요 — `buyer_profiles.business_name`만으론
  동명이인/동명업체 구분 불가).
- `groupDeals(deals)`: `formatDate(confirmed_at) + '__' + buyer_id`를 키로 `Map`에
  묶고, 그룹별로 `totalAmount`(합산), `status`(병합 — 분쟁중 > 진행중 >
  거래완료 우선순위로 "가장 안 끝난" 상태를 보여줌), 그룹 내부는
  최신순 정렬.
- `groupItemsSummary(group)`: 그룹에 속한 모든 거래의 견적 품목
  (`quotes.quote_requests.attributes.items`)을 합쳐서 요약 문자열 생성 —
  walk-in 거래(quotes 없음)만 섞인 그룹은 "통합 N건"으로 표시.
- 목록 행: 병합된 그룹이 1건 초과면 `▸`/`▾` 토글 버튼으로 펼침 —
  펼치면 그룹에 속한 개별 거래(날짜/품목/금액/상태)를 회색 배경 하위
  행으로 보여줌(하위 행엔 인쇄 링크 없음 — 인쇄는 그룹 단위로만).
- "명세서 인쇄" 링크: `/partner/dashboard/deals/${ids[0]}/invoice?ids=${ids.join(',')}`
  — 그룹에 속한 모든 deal id를 콤마로 이어 쿼리파라미터로 전달(병합
  안 된 단일 거래 그룹도 동일하게 `ids=<그 id 하나>`로 전달해서 로직을
  하나로 통일).

**`app/partner/dashboard/deals/[id]/invoice/page.tsx`**
- 기존엔 `useParams()`의 `id` 하나로만 조회하던 것을, `useSearchParams()`로
  `ids` 쿼리파라미터를 먼저 확인하고 있으면 그 목록을, 없으면 기존처럼
  `[id]` 하나만 쓰도록 확장 — **기존 북마크/링크(`ids` 없이 `[id]`만
  있는 URL)도 그대로 동작함**(하위 호환).
- `deals` 조회를 `.eq('id', dealId).maybeSingle()` → `.in('id', dealIds)`로
  변경. 받아온 여러 행 중 **첫 번째 행의 `buyer_id`와 다른 행은 걸러냄**
  (방어 로직 — `ids` 파라미터가 조작돼 다른 거래처 거래가 섞여 들어와도
  명세서 내용이 깨지지 않게 함; 애초에 그 거래를 볼 권한이 있는지는
  기존 RLS `deals_select_buyer_or_partner`가 이미 보장).
- 공급자/공급받는자 정보, `confirmed_at`(거래일자 표시), 문서번호(NO.)는
  필터링된 첫 거래 기준 그대로 사용, `amount`만 필터링된 거래들의 합으로
  재계산(단, 실제 렌더링에는 `amount` 필드 자체는 안 쓰이고 있었음 —
  총액은 항상 `deal_line_items`에서 다시 계산됨).
- `deal_line_items` 조회를 `.eq('deal_id', dealId)` → `.in('deal_id', matchedIds)`로
  확장 — 그룹에 속한 모든 거래의 품목이 `created_at` 순으로 한 표에
  합쳐짐. 외상잔액(`ar_balances`) 조회는 원래도 거래 단위가 아니라
  파트너+소상공인 단위(현재 누적잔액)라 변경 불필요.
- **인쇄 레이아웃(A4 1장, 공급자/공급받는자 절반씩, 점선 절취선, 2열
  정보박스 등) 자체는 한 줄도 안 건드림** — 지시대로 데이터 조회 범위만
  확장.
- `useSearchParams()`를 쓰려면 이 Next.js 버전에서 `<Suspense>` 경계가
  필요해서(다른 기존 페이지들 — `app/search/page.tsx` 등 — 도 동일 패턴),
  본문 로직을 `DealInvoicePageInner`로 분리하고 `export default`는
  `<Suspense fallback=...><DealInvoicePageInner /></Suspense>`로 감쌈.

### 실제 검증 (실데이터로 같은 날짜 거래 3건 직접 생성해서 확인)

RLS 구조상(`deals_insert_buyer`: buyer 본인만 deals insert 가능,
`deal_line_items_insert_partner`: partner 본인만 line item insert 가능)
Node 스크립트로 test01@test.com(all식당, buyer) 세션에서 `deals` 3건을
같은 `confirmed_at`(현재 시각)·같은 `partner_id`(test4@email.com,
test공급식자재)로 직접 insert하고, test4 세션으로 각 거래에 품목을
1개씩(테스트-소스A/B/C, 그 중 B는 `is_credit=true`) insert함 — anon key로
서비스 롤 없이, 실제 앱이 쓰는 것과 동일한 RLS 경로로 데이터를 만듦.

브라우저(puppeteer-core, `test4@email.com`)로 확인:
- "진행 중인 거래" 목록에 `2026.09.09 / all식당 / 통합 3건 / 95,000원 /
  진행중 / 명세서 인쇄 (3건 통합)`로 **정확히 한 행으로 병합**됨(다른
  날짜의 기존 실거래는 그대로 별도 행 유지).
- `▸` 클릭 → 개별 거래 3건(20,000/45,000/30,000원, 날짜 동일)이 하위
  행으로 펼쳐짐 확인.
- "명세서 인쇄 (3건 통합)" 클릭 → 인쇄 페이지 URL이
  `.../invoice/<id1>?ids=<id1>,<id2>,<id3>`로 생성됨 → 렌더된 품목표에
  테스트-소스A/B/C **3건이 한 표에 모두** 나타나고, 합계(95,000원),
  전미수금/총미수금/금일입금/총합계 계산도 정확함(스크린샷으로 레이아웃도
  기존 v9 그대로 유지되는 것 확인 — 2열 정보박스 나란히 배치, 절취선
  정중앙 등 회귀 없음).
- 병합 안 된 기존 단일 거래(`.../invoice/37dbc5d5-...?ids=37dbc5d5-...`)도
  그대로 정상 렌더 — 하위 호환 확인.
- 콘솔 에러 0건, `tsc --noEmit` 새 에러 없음.

**테스트 데이터 정리**: 위 검증용으로 만든 `deals` 3건은
`settlements_generate_on_deal_insert` 트리거로 `settlements` 3건도 같이
생겼고, `is_credit=true` 품목 하나가 `ledgerbook_on_line_item_insert`
트리거로 `ar_balances`를 100,000→145,000원으로 올려둠(재고 차감은
품목명이 `stock_levels`에 등록돼 있지 않아 영향 없음 — 사전 조회로 확인).
`supabase/migrations/20260918000000_cleanup_invoice_grouping_test_data.sql`
로 `deal_line_items`/`settlements`/`deals` 삭제 + `ar_balances`를
`balance = balance - 45000`으로 원복(절대값 덮어쓰기 아님 — 그 사이 실제
거래가 더 생겨도 안전) — 사용자가 실행 후 "Success. No rows returned"로만
끝나는 걸 그대로 믿지 않고, 실행 뒤 별도 읽기 전용 조회로 `deals`/
`deal_line_items`/`settlements` 0건, `ar_balances` 100,000원 복귀까지
직접 재확인함.

### 이번에 하지 않은 것

- `settlements`(정산)는 여전히 거래(`deals`) 단위로 개별 생성됨 — 그룹
  단위로 합치지 않음(지시 범위가 "화면 조회/명세서 인쇄" 한정이라 정산
  로직은 안 건드림). 정산도 날짜+거래처 단위로 합쳐야 하는지는 별도
  확인 필요.
- 그룹 상태 병합은 "가장 안 끝난 상태 우선"(분쟁중 > 진행중 > 거래완료)
  단순 규칙만 적용 — 그룹 안에 상태가 섞였을 때 더 정교한 표시(예: "2건
  완료, 1건 진행중")는 펼침 보기로만 확인 가능.
- 문서번호(NO.)는 그룹의 첫 번째 거래 id 기준 그대로 씀(그룹 전용 번호
  체계는 새로 안 만듦) — 그룹 구성이 페이지 재조회마다 바뀔 수 있는
  요소는 아니라(같은 날짜+거래처 조합은 고정) 문제는 없지만, DB가 어떤
  순서로 행을 반환하는지에 따라 "첫 번째"가 달라질 수는 있음(정렬 순서
  명시 안 함 — 인쇄용 참고번호일 뿐이라 실질적 영향 없음).
- `/partner/ledger`의 "일자별 거래전표 내역"은 이번에 그룹핑을 추가하지
  않음(원래 목적이 달라 손대지 않음 — 위 "확인이 필요하다고 한 두 가지"
  참고).

## 거래처 주문 마감시간 설정

지시 배경: 납품업체는 통상 전날 마감시간까지 받은 주문을 모아 익일부터
배송 준비를 함 - 마감시간이 지난 주문은 자동으로 다음 배송 가능일로
넘어가야 하고, 이걸 거래처(소상공인)마다 다르게 설정할 수 있어야 함
(경쟁사 마켓봄 사례: 배송요일+마감시간을 묶은 "주문그룹"에 거래처를
배정).

### 구현 전 조사 (마이그레이션 전에 먼저 확인받은 부분)

1. **배송예정일류 컬럼 존재 여부**: `quote_requests`/`quotes`/`deals`/
   `deal_line_items` 실제 컬럼을 라이브 쿼리로 확인한 결과 전혀 없었음
   (`quotes.eta_or_schedule`는 자유 텍스트라 구조화된 날짜가 아님).
2. **마감시간 계산이 적용돼야 할 실제 진입점**: `deals`는 buyer 세션
   에서만 insert 가능한 RLS 구조(`deals_insert_buyer`: `qd_is_my_buyer_id
   (buyer_id)`만 허용, partner insert 정책 자체가 없음)라 매일 반복
   발주 때마다 새로 만들어지지 않음 - quote_requests→견적 확인→`deals`
   1건 생성은 "거래관계를 처음 트는" 1회성 이벤트고, 그 이후 매일의
   실제 발주는 **기존 deal에 `deal_line_items`를 추가하는 것**(거래전표
   등록/AI 빠른입력)임. `DealPicker`(두 화면 공용)도 기존 진행중 거래를
   고르거나 신규 거래처를 만드는 구조일 뿐, 기존 거래처에 새 `deals`를
   매번 만들지 않음. **결론: `deal_line_items` insert 시점 한 곳에만
   적용**(quote_requests/quote-compare는 그 시점에 아직 파트너가
   확정 안 돼 있어 어느 그룹인지도 알 수 없고, 반복 발주 마감 개념과도
   안 맞아 제외).
3. **"새 거래처로 시작하기"(walk-in) 연동**: `createWalkInDeal()`은
   `deals`만 만들고 `deal_line_items`는 안 만듦(그 직후 같은 그리드에서
   품목을 입력하게 됨) - 저장 경로가 위 2번과 동일해서 특별 처리 없이
   자동으로 맞물림. 신규 거래처는 배정 이력이 없으니 자동으로 "그룹
   미지정=제한없음" 기본값이 적용됨.

이 세 가지를 정리해 사용자에게 먼저 보고하고, 승인받은 뒤에 마이그레이션을
작성함(사용자가 명시적으로 "마이그레이션 실행 전에 먼저 확인받고 진행"을
요청했던 부분).

### DB (`20260919000000_partner_order_groups.sql`)

- `partner_order_groups(id, partner_id, group_name, delivery_days
  smallint[], cutoff_time time, allow_after_cutoff boolean,
  created_at)` - `delivery_days`는 0=일~6=토(JS `Date.getDay()`와 동일
  숫자 체계). RLS: 본인 파트너 것만 select/insert/update/delete.
- `partner_buyer_order_groups(id, partner_id, buyer_id, order_group_id,
  updated_at)`, `unique(partner_id, buyer_id)` - 거래처 배정을
  `buyer_profiles`에 컬럼을 추가하는 대신 별도 매핑 테이블로 분리함
  (같은 소상공인도 파트너마다 다른 그룹에 배정될 수 있어야 하므로).
  `order_group_id`는 `on delete cascade`라 그룹을 지우면 배정 행도
  자동으로 같이 지워져 "미지정" 상태로 복귀함(배정 없음 행이 곧
  "미지정"이라 null 허용 컬럼으로 만들지 않음 - 미지정은 행 자체가
  없는 것으로 표현).
- `deal_line_items.expected_delivery_date`(nullable date) 컬럼 추가만
  - 기존 컬럼/트리거는 전혀 안 건드림.

### 계산 로직 (`lib/orderGroups.ts`, 순수 함수)

`computeExpectedDelivery(group, now)`:
- **마감 전**(`now` ≤ 오늘 `cutoff_time`): 오늘부터(오늘 포함) 가장
  빠른 배송요일.
- **마감 후**: 오늘은 건너뛰고 그 다음 배송요일(오늘이 배송요일이어도
  이미 마감이라 스킵).
- **`allow_after_cutoff=true`인 그룹**: 이 계산 자체를 건너뛰고 `null`
  반환 - 호출부가 "기존처럼 즉시 처리"로 취급(지시 4번 요구사항 그대로).
- `delivery_days`가 비어 있거나(그룹 설정 오류) 14일 안에 배송요일이
  하나도 안 걸리면 `null`(제한 없음으로 취급).
- `fetchOrderGroupForBuyer(partnerId, buyerId)`: `partner_buyer_order_
  groups`를 `partner_order_groups`와 조인해서 조회 - 배정이 없으면
  `null`(제한 없음, 기존 동작 그대로 - 지시 2번 "기본값은 그룹 미지정
  시 마감시간 제한 없음").

### 화면

- **`/partner/order-groups`**("거래처 주문그룹 관리", 사이드바에서
  "프로필·배송조건 관리" 바로 아래 배치) - 주문그룹 생성/수정/삭제 폼
  + 요일은 칩(chip) 다중 선택, 마감시간은 `<input type="time">`. 아래에
  이 파트너와 실제 거래 이력이 있는 거래처 목록(= `deals`에서 distinct
  `buyer_id`)을 보여주고, 각 행에 드롭다운으로 그룹 배정/미지정 전환
  (미지정 선택 시 `partner_buyer_order_groups` 행을 delete, 그룹 선택
  시 `upsert(..., {onConflict:'partner_id,buyer_id'})`).
- **`DeliveryEstimateBanner`**(신규 공용 컴포넌트) - `거래전표 등록`
  (`ledger-entry`)과 `AI 빠른입력`(`AiQuickEntryModal`) 양쪽에서 거래
  선택 직후 표시. 4가지 문구: 그룹 미지정/시간외 허용/마감 전(예상
  배송일 안내)/마감 후(이월 안내). 두 화면 모두 이미 갖고 있던 `deals`
  목록에 `buyer_id`를 추가해서(기존엔 `business_name`만 있었음) 선택된
  거래의 buyer_id로 그룹을 조회 - `DealPicker`의 `DealOption` 타입에도
  `buyer_id`를 추가해서 두 화면이 공용으로 씀.
- 저장 시(`handleGridSave`/`handleConfirm`) **저장 버튼을 누른 시점
  기준으로 다시 계산**해서 `expected_delivery_date`에 저장(그리드에
  입력하는 동안 마감시간을 넘길 수도 있으므로, 배너에 보여준 값을
  그대로 안 믿고 저장 직전 재계산).
- `ledger-entry`의 전표 이력 표에 "예상 배송일" 열을 추가해서, 이번
  기능 이전에 등록된 기존 품목(값 없음)은 "-"로, 이번에 새로 등록된
  품목은 계산된 날짜로 표시됨(하위 호환).

### 실제 검증 (puppeteer-core, test4@email.com × all식당)

4가지 케이스 전부 브라우저에서 실제 그룹을 만들고/수정하고/배정하며
확인:
- **그룹 미지정**(배정 전): "이 거래처는 주문그룹이 지정되지 않았어요
  — 마감시간 제한 없이 바로 처리돼요."
- **마감 전**(마감시간을 23:59로 설정, 현재 시각이 그 전): `"과거마감
  테스트" 예상 배송일: 9/9(수) (마감시간 23:59 전)` - 오늘 날짜로 계산됨.
- **마감 후**(마감시간을 00:01로 설정, 현재 시각이 이미 지남): `"과거
  마감테스트" 마감시간(00:01)이 지나 9/10(목) 배송으로 예약돼요.` -
  다음날로 이월됨.
- **시간외 허용**(`allow_after_cutoff` 체크): `"과거마감테스트" 그룹은
  시간외 주문이 허용돼 있어 마감시간과 무관하게 바로 처리돼요.`
- 마감 후 케이스 상태에서 실제로 품목("마감테스트품목")을 그리드에
  입력해 저장 → 전표 이력 표의 "예상 배송일" 열에 정확히 `2026.09.10`
  으로 저장·표시되는 것까지 DB 직접 조회로 재확인. 기존(이 기능 이전)
  품목 3건은 "-"로 그대로 남아있어 하위 호환도 확인.
- `tsc --noEmit` 새 에러 없음.

**AI 빠른입력 쪽은 코드 재사용 구조로만 검증**: `AiQuickEntryModal`도
동일한 `computeExpectedDelivery`/`fetchOrderGroupForBuyer`/
`DeliveryEstimateBanner`를 그대로 재사용하고, `dealId` 변경 시 그룹을
조회하는 `useEffect`도 ledger-entry와 구조적으로 동일하게 작성함 - 다만
Step 3(리뷰 화면)까지 도달하려면 실제 Claude API 호출(과금)이 필요해서,
이번엔 실제 AI 파싱까지는 안 돌려보고 구조적 동일성만으로 검증을
갈음함. 다음에 이 모달을 실제로 만질 일이 있으면 이 부분도 실제 카톡
문구로 한 번 확인해볼 것.

**테스트 데이터 정리**: 위 검증용으로 만든 주문그룹("과거마감테스트"),
거래처 배정(all식당→그 그룹), 테스트 품목 1건("마감테스트품목", 3,000원,
is_credit=false)을
`supabase/migrations/20260920000000_cleanup_order_groups_test_data.sql`
로 정리 - 그룹을 지우면 배정 행은 `on delete cascade`로 자동 같이
지워짐(위 DB 섹션 참고), `deal_line_items`만 별도로 지우면 됨.

### 이번에 하지 않은 것

- `/partner/ledger`(매출·재고 현황)이나 소상공인 쪽 화면에는 예상
  배송일을 아직 안 보여줌 - 지시의 "해당되면 동일하게 표시"는 우선
  핵심 진입점(거래전표 등록/AI 빠른입력)에만 적용하고, 조회 전용 화면
  확장은 다음 범위로 미룸.
- 그룹 삭제 시 확인 다이얼로그는 `window.confirm()`(이 프로젝트의 기존
  삭제 확인 패턴, `admin/categories` 등과 동일) - 별도 커스텀 모달은
  안 만듦.
- "내일 배송 목록" 같은 `expected_delivery_date` 기반 조회 화면은 이번
  범위 밖(컬럼만 저장해두고, 활용하는 화면은 다음 기회에).
- 배송요일 요일별 캐파(용량) 제한, 그룹별 우선순위, 마감시간 임박 알림
  등은 마켓봄 사례에 있을 수 있지만 지시에 없어서 구현 안 함.

## 다음에 할 만한 것 (제안, 확정 아님)

- `expected_delivery_date` 기반 "내일 배송 목록"(공급업체가 다음 배송일
  기준으로 품목을 모아볼 수 있는 조회 화면) - 컬럼은 이번에 저장해뒀지만
  활용 화면은 아직 없음
- 예상 배송일을 `/partner/ledger`, 소상공인 마이페이지/견적 관련 화면에도
  노출할지 검토(이번엔 핵심 진입점인 거래전표 등록/AI 빠른입력에만 적용)
- "AI 빠른입력"의 `DeliveryEstimateBanner` 노출을 실제 Claude API 호출로
  end-to-end 검증(이번엔 구조적 동일성만으로 검증함 - 위 "거래처 주문
  마감시간 설정" 섹션 참고)
- `LineItemGrid`에 복사/붙여넣기(엑셀에서 여러 셀 복사해서 한 번에
  붙여넣기) 지원 - 마켓봄류 프로그램에서 자주 쓰이는 패턴이지만 이번
  범위엔 없었음
- `LineItemGrid`에 위/아래 화살표 키로 셀 이동 지원(지금은 Tab/Enter만)
